...
def extract_coastline(x_mod,y_mod,input_file,output_file):
	...
	ncst=np.delete(ncst, np.arange(k[l+1], ncst.size/2).astype(int), axis=0) # get rid of unused part of data matrices
	Area=np.delete(Area, (np.arange((l+1), Area.size)), axis=0)
	k=np.delete( k , ( np.arange ( (l+2) , k.size)), axis=0)
	...
	for t in IX:
		...
		for r in np.arange(0,iCoastalSection.size/2,1):
			...
			rr = int(r)
			CoastFile.write("%-8.5f %-6.5f \n" % (iCoastalSection[rr,0], iCoastalSection[rr,1]))
...
# - gshhs_filename: set path for your original gebco netCDF file
...
if len(sys.argv) > 1:
        ...
        with open(configFile) as f:
                try:
                        config = json.load(f)
                        gshhs_filename = config["preproc_gshhs"]["gshhs_filename"]
                        output_dir = config["preproc_gshhs"]["output_dir"]
                        oce_dir = config["preproc_gshhs"]["oce_dir"]
                except KeyError:
...
# extract coastline
output_file = output_dir + "/vnzl.map"
