...
c	if(regn1.eq.'medf') then
c        iregn=0
      if(regn1.eq.'vnzl') then
        iregn=0        
...
c----------------------------------------------------------------------
	if(icurrents.ge.1.and.icurrents.le.80) then
        ...
        inquire(file='../DTM_INP/newregions.txt',EXIST=ex)
	  if(ex) then
          open(1,file='../DTM_INP/newregions.txt')
          read(1,*) nnew
          do k=1,nnew
            read(1,'(a4)') a4
            read(1,'(a80)') empty
...
c----------------------------------------------------------------------
      ...
      if(icencr.ne.0) then
        open(50,file='../DTM_INP/'//regn1//'_.bath',status='old')

	else
        open(50,file='../DTM_INP/'//regn1//'.bath',status='old')
...
c        call calcfetch(xavg,yavg,wdirstoke,fetch)
        call calcfetch(xavg,yavg,wdirstoke,fetch,regn1) ! 2.01 regn1 passed as parameter
	xavg_lon=glon(xavg)
	yavg_lat=glat(yavg)
...
c
	...
	common /encr/ aloncorr,iaencr,ibencr,icencr
        ...
        open(51,file='../DTM_INP/'//regn1//'.map',status='old')

	
      open(52,file='../DTM_INP/'//regn1//'cst1.d')
...
c
	...
	if(ex) then
        ...
        open(80,file='../DTM_INP/'//regn1//'.cst')
        do k=1,11
          read(80,'(a80)') empty
        end do
        k = 0    
...
c      
     ...
     &                  '../DTM_INP/'//regn1//'clim.sst'
        open(54,file='../DTM_INP/'//regn1//'clim.sst',status='old')
        read(54,'(a80)') empty
        read(54,'(a80)') empty
...
c
	...
	if(iwind.eq.5) dirr='TEMP//////MET/'							!str
...
c       (R. Lardner, M. De Dominicis, 2009)
c----------------------------------------------------------------------
c	subroutine calcfetch(xavg,yavg,wdirstoke,fetch)
	subroutine calcfetch(xavg,yavg,wdirstoke,fetch,regn1) ! 2.01 regn1 passed as parameter
...
c     &          angledeg(8), fetch(imx,jmx,8)
      ...
      character regn1*4 ! 2.01 now there is a coastline of our region
...
c	open(1,file='data/'//regn//'.map')  ! 2.01 there was the Med only 
	open(1,file='../DTM_INP/'//regn1//'.map') ! 2.01 now there is a coastline of our region
      read(1,*) ncontours
