...
simname        = 'test4'      ### Simulation name - format (string)
sim_date       = '09/10/2023' ### Simulation start day  - format DD/MM/YYYY (string)
sim_hour       = '12:00'      ### Simulation start hour - format HH:mm (string)
...
oil_api        = 34           ### Oil API - format (float)
oil_volume     = 100            ### Volume of oil in tons - format (float) 
...
if __name__ == '__main__':
    ...
    my_o = xr.open_dataset(glob(f'cases/{simname}/oce_files/*nc')[0]).isel(depth=0,time=0).uo.values.shape
    ...
    subprocess.run([f"{config} sed -i s/HH/{dt_sim.hour:02d}/ {config_file}"],shell=True)
    subprocess.run([f"{config} sed -i s/mm/{dt_sim.minute:02d}/ {config_file}"],shell=True)   
