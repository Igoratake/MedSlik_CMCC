...
def parallel_processing_mrc(concat):
    num_time_steps = len(concat.time)
    pool = multiprocessing.Pool()  # Create a pool of workers
    results = [pool.apply_async(process_mrc, args=(i, concat)) for i in range(num_time_steps)]
...
if __name__ == '__main__':
    ...
    parallel_processing_mrc(concat)          
