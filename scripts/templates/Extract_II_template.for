...
c-----------------------------------------------------------------------------------
      ...
      character indate(30)*8,indate_wind(30)*8, fc_dir*120
      common alon1, alon2, alat1, alat2, numfiles, indate,
	...
	do n=1,numfiles !modif

	  read(1,'(a8)') indate(n)
	enddo
	read(1,*) numfiles_wind

	do n=1,numfiles_wind !orig
	...
	close(1)
      ...
      call ExtractCURR(fc_dir,len_dir)
      call ExtractWIND(fc_dir,len_dir)

      stop
	end
c********************************************************************
c     Extract and transform ocean fields
c********************************************************************
      subroutine ExtractCURR(fc_dir,len_dir)

      parameter(ktmx=24, imx=XCURR, jmx=YCURR, kmx=4)
...
c other variables
     ...
     &          heads*150, empty*80, ora*2, ore*2, fc_dir*120,
     &          start_lat*19
      logical ex
      integer t,len_dir
      common alon1, alon2, alat1, alat2, numfiles, indate,
...
c--------------------------------------------------------------------
      ...
      Status = 0
      infile=fc_dir(1:len_dir)//'/PREPROC/OCE/'
     &                                //indate(n)(1:6)//'_U.nc'
...
c     extract data from u file
      retval = nf_get_var_real(ncid, u_varid,u_24)
      if (retval .ne. nf_noerr) call handle_err(retval)
...
c     extract data from v file
      retval = nf_get_var_real(ncid, v_varid,v_24)
      if (retval .ne. nf_noerr) call handle_err(retval)
...
c     extract data from t file
      retval = nf_get_var_real(ncid, t_varid,ts_24)
      if (retval .ne. nf_noerr) call handle_err(retval)
     ...
     &                  indate(n)(1:2)//' '//ora//':00'
        write(6,*) 'Writing ocean fields for date '//prdate
        write(99,*) 'Writing ocean fields for date '//prdate
        outfile =
     & 'TEMP/OCE/'//'merc'//indate(n)(1:6)//ora(1:2)//'.mrc'
...
c--------------------------------------------------------------------
     ...
     & 'TEMP/OCE/'//'merc'//indate(n)(1:6)//ora(1:2)//'.mrc'
      open(20,file = outfile)
        write(20,*) 'Ocean forecast data for '//prdate
...
c********************************************************************
c     Extract and transform WIND fields
c********************************************************************
      subroutine ExtractWIND(fc_dir,len_dir)

       parameter(ktmx=24, imx=XWIND, jmx=YWIND)
...
c  main program
      ...
      infile=fc_dir(1:len_dir)//'/PREPROC/MET/'
...
c--------------------------------------------------------------------
     ...
     &                  indate(n)(3:4)
        write(6,*) 'Writing user winds for date '//prdate
        write(99,*) 'Writing user winds for date '//prdate
        outfile =
     & 'TEMP/MET/'//'erai'//indate(n)(3:8)//'.eri'
...
c--------------------------------------------------------------------
     ...
     & 'TEMP/MET/'//'erai'//indate(n)(3:8)//'.eri'
      open(20,file = outfile)
        write(20,*) '10m winds forecast data for '//prdate
