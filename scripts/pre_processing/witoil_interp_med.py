...
def load_oceanfields(sFileName):
	...
	iTime=fM.variables["time_counter"][:]  # seconds since 1970-01-01 00:00:00
...
# this is your current date in seconds since 1970-01-01 00:00:00
currdate = (date(int(YYYY),int(MM),int(DD)).toordinal() - date(1970,1,1).toordinal())*24
...
time_input = time_input/3600. # hours since 1970-01-01
	...
	for jj in range(0,len(x)):
		for dd in range(0,len(depth)):
			...
			if var[0,dd,ii,jj]>-20:
				...
				#Extrapolating data
				if (dd!=0 and any(out_field[:,dd,ii,jj]<-2) and all(out_field[:,dd-1,ii,jj]>-2)):
...
f.history = 'IBI-ROOS current fields :: adapted to Medslik-II'
...
time.units = 'hours since 1970-01-01 00:00'
