...
if __name__ == '__main__':
    ...
    path = '/Users/iatake/Dropbox (CMCC)/Work/MEDSLIK-II and Pyslick/Medslik-II_results/ORIGINAL_DATA/ELBA/'

    ds_particles = xr.open_dataset(path + 'spill_properties_merged.nc')
    ds_concentration = xr.open_dataset(path + 'spill_properties_concentration.nc')

    lon_min, lon_max = 9.8,10.2
    lat_min, lat_max = 42.8,43.2
    ...
    for t in range(0,24):
        ...
        for plot in ['particle','concentration']:
            ...
            if plot == 'particle':
                ...
                ds_c = xr.where(ds_c.concentration > 0.1, ds_c.concentration,np.nan)
                ds_c.plot(ax=ax)
            ...
            plt.savefig(path + f'figures/fig_elba_{t:03d}_{plot}.png',dpi=100)
