...
# 8. SCALAR FIELD SETUP ---------------------------------------------------------------------
    ...
    cfres.cnLevels                  = [ 0.1,0.2,0.5,0.8,1.1,1.2,1.5,1.8,2.1,2.2,2.5,2.8,5.1,5.2,
                                        5.5,5.8,8.1,8.2,8.5,8.8,10.1,10.2,10.5,10.8,12.1,12.2,12.5,12.8]                                

    cfres.lbOrientation              = "Horizontal"
    cfres.lbTitleString              = "Surface oil concentration, ton/km^2"
...
# 9. SAMPLE MARKER SETUP --------------------------------------------------------------------
    ...
    m1res.gsLineThicknessF          = 8.0
    m1res.gsLineColor               = "black"
...
# 16. IF PRESENT, PLOT SAMPLE OBSERVATIONS ----------------------------------------------------
    ...
    if t == 25:
       ...
       print M2
       for i in range(0,6):
           print M1[i][1], M1[i+1][1]
           Ngl.add_polyline(wks,mp,[M1[i][1],M1[i+1][1]],[M1[i][0],M1[i+1][0]],m1res)
           Ngl.add_polyline(wks,mp,[M2[i][1],M2[i+1][1]],[M2[i][0],M2[i+1][0]],m1res)
...
# 19. Draw text strings after the plot is drawn to make sure plot gets maximized properly --------
    ...
    if (t < 10) :
        ...
        string2 = "+ 00" + str(t)
    string = string1 + " 09:51 UTC " + string2 + " hours "
