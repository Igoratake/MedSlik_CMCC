...
# - time_index: time steps of interest (hours by default -- Python counting starts from 0).
#   It may be a single number e.g. [146] or a list of numbers e.g. np.arange(0,15)
# - coast_file: where your coastline-support file is located
...
if len(sys.argv) > 1:
        ...
        with open(configFile) as f:
                try:
                        config = json.load(f)
                        sNcFile = config["oil_beached"]["spill_prop_file"]
                        output_folder = config["oil_beached"]["output_folder"]
                        coast_file = config["oil_beached"]["coast_file"]
                        st = config["oil_beached"]["time_line"]["start"]
                        en = config["oil_beached"]["time_line"]["end"]
                        inc = config["oil_beached"]["time_line"]["increment"]
                        time_index = np.arange(st, en, inc)
                except KeyError:
